...
describe("workspace_factory", function()
  describe("building a workspace from a json string successfully", function()
    it("should return a workspace", function()
      ...
      assert.are.same(workspace.tabpages[1].tagpageId, 1)
      assert.are.same(workspace.tabpages[1].windows[1].windowId, 1)
      assert.are.same(workspace.tabpages[1].windows[1].buffer.bufferId, 1)
    end)
  end)
end)
